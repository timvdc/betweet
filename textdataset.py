...
class TextDataset(Dataset):
    def __init__(self, tokenizer, model_name_or_path, df, labels, block_size=512, overwrite_cache=True, mask_padding_with_zero=True):
        ...
        if os.path.exists(cached_features_file) and not overwrite_cache:
            ...
            for row,label in tqdm(zip(df.iterrows(),labels), total=len(df), mininterval=1, position=1, leave=True):
                index = row[0]
                sentence = row[1]['text']
                ...
                self.examples.append([tokenized_text[0 : block_size - 3], input_mask[0 : block_size - 3], [label]])
...
def load_and_cache_examples(model_name_or_path, tokenizer, df, labels):
    dataset = TextDataset(
        tokenizer,
        model_name_or_path,
        df,
